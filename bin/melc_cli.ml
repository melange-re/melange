...
module Internal = struct
  ...
  let delay_js_optimizations =
    Arg.(
      value & vflag None
      & [
          ( Some true,
            info ~doc:"*internal* Delay JavaScript optimizations until emission"
              [ "mel-delay-js-optimizations" ] );
          ( Some false,
            info
              ~doc:
                "*internal* Run JavaScript optimizations during CMJ generation"
              [ "mel-eager-js-optimizations" ] );
        ])
...
let parse include_dirs hidden_include_dirs alerts warnings output_name ppx
    ...
    bs_no_version_header bs_cross_module_opt delay_js_optimizations action_trace
    bs_diagnose where verbose keep_locs bs_no_check_div_by_zero bs_noassertfalse
    noassert bs_loc impl intf intf_suffix cmi_file g opaque preamble
    strict_sequence strict_formats dtypedtree dparsetree drawlambda dsource
    version pp absname bin_annot i nopervasives nolabels principal rectypes
    short_paths unsafe warn_help warn_error bs_stop_after_cmj runtime filenames
    _c store_occurrences =
  {
    include_dirs;
    hidden_include_dirs;
    alerts;
...
let cmd =
  Term.(
    ...
    $ Internal.delay_js_optimizations $ Internal.action_trace
