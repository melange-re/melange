...
let map_quoted_string (env : env) (tok : CST.quoted_string) =
  token env tok (* quoted_string *)
...
let map_constant (env : env) (x : CST.constant) =
  ...
  | `Quoted_str_ (v1, v2, v3) ->
      let v1 = token env v1 (* "{" *) in
      let v2 = token env v2 (* quoted_string *) in
      let v3 = token env v3 (* "}" *) in
      todo env (v1, v2, v3)
...
and map_anon_choice_cons_type_48a9640 (env : env) (x : CST.anon_choice_cons_type_48a9640) =
  (match x with
...
and map_extension (env : env) (x : CST.extension) =
  ...
  | `Quoted_exte (v1, v2, v3, v4, v5) ->
      ...
      let v3 =
        ...
        | None -> todo env ())
      in
      let v4 = token env v4 (* quoted_string *) in
      let v5 = token env v5 (* "}" *) in
      todo env (v1, v2, v3, v4, v5)
  )
...
and map_item_extension (env : env) (x : CST.item_extension) =
  ...
  | `Item_exte_ (v1, v2, v3, v4, v5) ->
      ...
      let v5 = List.map (map_item_attribute env) v5 in
      todo env (v1, v2, v3, v4, v5)
  | `Quoted_item_exte (v1, v2, v3, v4, v5, v6) ->
      let v1 = token env v1 (* "{%%" *) in
      let v2 = map_attribute_id env v2 in
      let v3 =
        (match v3 with
...
and map_let_binding (env : env) ((v1, v2, v3, v4, v5, v6, v7) : CST.let_binding) =
  let v1 = map_binding_pattern_ext env v1 in
  let v2 = List.map (map_parameter env) v2 in
  let v3 =
    (match v3 with
...
and map_module_type (env : env) (x : CST.module_type) =
  ...
  | `Module_type_cons (v1, v2, v3, v4) ->
      ...
      let v3 = map_anon_choice_cons_type_48a9640 env v3 in
      let v4 =
        List.map (fun (v1, v2) ->
          let v1 = token env v1 (* "and" *) in
          let v2 = map_anon_choice_cons_type_48a9640 env v2 in
          todo env (v1, v2)
        ) v4
      in
      todo env (v1, v2, v3, v4)
...
and map_parameter_ (env : env) (x : CST.parameter_) =
  ...
  | `Choice_TILDE_LPAR_id_opt_typed_opt_EQ_seq_exp_ext_RPAR (v1, v2, v3, v4, v5, v6) ->
      ...
      let v6 = token env v6 (* ")" *) in
      todo env (v1, v2, v3, v4, v5, v6)
  | `Label_imm_tok_COLON_LPAR_pat_ext_opt_typed_EQ_seq_exp_ext_RPAR (v1, v2, v3, v4, v5, v6, v7, v8) ->
      let v1 = map_label env v1 in
      let v2 = token env v2 (* ":" *) in
      let v3 = token env v3 (* "(" *) in
