...
function getExn(arr, i) {
  if (!(i >= 0 && i < arr.length)) {
    throw Caml_exceptions.stacktrace(new Error("File \"belt_Array.ml\", line 25, characters 6-12"));
  }
  return arr[i];
}
...
function setExn(arr, i, v) {
  if (!(i >= 0 && i < arr.length)) {
    throw Caml_exceptions.stacktrace(new Error("File \"belt_Array.ml\", line 31, characters 4-10"));
  }
  arr[i] = v;
  return /* () */0;
}
