...
var dummy_met = Caml_external_polyfill.resolve("caml_obj_block")(0, 0);
...
function new_table(pub_labels) {
  ...
  var methods = Caml_array.make((len << 1) + 2 | 0, dummy_met);
...
function resize(array, new_size) {
  ...
  var new_buck = Caml_array.make(new_size, dummy_met);
...
function init_class(table) {
  ...
  return resize(table, 3 + ((Caml_array.get(table.methods, 1) << 4) / Sys.word_size | 0) | 0);
}
...
function create_object(table) {
  var obj = Caml_external_polyfill.resolve("caml_obj_block")(Obj.object_tag, table.size);
  obj[0] = table.methods;
...
function new_cache(table) {
  ...
  var n$1 = n % 2 === 0 || n > (2 + ((Caml_array.get(table.methods, 1) << 4) / Sys.word_size | 0) | 0) ? n : new_method(table);
...
function set_methods(table, methods) {
  ...
  while(i[0] < len) {
    var label = Caml_array.get(methods, i[0]);
