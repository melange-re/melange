...
function Make(H) {
  ...
  var next_sz = function (n) {
    return Caml.caml_int_min((Math.imul(3, n) / 2 | 0) + 3 | 0, Sys.max_array_length);
  };
  ...
  var add_aux = function (t, setter, d, h, index) {
    ...
    while(true) {
      var i = _i;
      if (i >= sz) {
        var newsz = Caml.caml_int_min((Math.imul(3, sz) / 2 | 0) + 3 | 0, Sys.max_array_length - 2 | 0);
        if (newsz <= sz) {
          throw {
                RE_EXN_ID: "Failure",
  ...
  var stats = function (t) {
    ...
    $$Array.sort(Caml.caml_int_compare, lens);
