...
function pp_force_break_line(state) {
  ...
  if (width > state.pp_space_left && (match$1.box_type - 1 >>> 0) <= 3) {
  ...
  if (typeof s === "number") {
    switch (s) {
      case /* Pp_stab */0 :
          ...
          var tabs = Caml_option.valFromOption(match)._0;
      ...
      case /* Pp_newline */3 :
          ...
          if (match$1 !== undefined) {
            return break_line(state, Caml_option.valFromOption(match$1).width);
          } else {
            return Curry._1(state.pp_out_newline, undefined);
          }
      case /* Pp_if_newline */4 :
          if (state.pp_current_indent !== (state.pp_margin - state.pp_space_left | 0)) {
            ...
            var size$1 = match$3.size;
            state.pp_left_total = state.pp_left_total - match$3.length | 0;
    ...
    switch (s.TAG | 0) {
      ...
      case /* Pp_break */1 :
          ...
          var before = breaks[0];
          var fits = s.fits;
          var match$4 = Stack.top_opt(state.pp_format_stack);
          if (match$4 === undefined) {
            return ;
          }
          var match$5 = Caml_option.valFromOption(match$4);
          var width = match$5.width;
          switch (match$5.box_type) {
            case /* Pp_vbox */1 :
            case /* Pp_hvbox */2 :
      ...
      case /* Pp_tbreak */2 :
          ...
          var match$6 = Stack.top_opt(state.pp_tbox_stack);
          if (match$6 === undefined) {
            return ;
          }
          var tabs$1 = Caml_option.valFromOption(match$6)._0;
          var match$7 = tabs$1[0];
          var tab;
          if (match$7) {
            var first = match$7.hd;
...
function advance_left(state) {
  while(true) {
    ...
    format_pp_token(state, size$1, match$1.token);
    state.pp_left_total = match$1.length + state.pp_left_total | 0;
    continue ;
  };
}
...
function set_size(state, ty) {
  ...
  var match$1 = Caml_option.valFromOption(match);
  var queue_elem = match$1.queue_elem;
  var size = queue_elem.size;
  if (match$1.left_total < state.pp_left_total) {
    return initialize_scan_stack(state.pp_scan_stack);
  }
  var match$2 = queue_elem.token;
...
function pp_print_list(_$staropt$star, pp_v, ppf, _param) {
  while(true) {
    ...
    var pp_sep = $staropt$star !== undefined ? Caml_option.valFromOption($staropt$star) : pp_print_cut;
    if (!param) {
      return ;
    }
    var v = param.hd;
...
function pp_print_seq(pp_sepOpt, pp_v, ppf, seq) {
  var pp_sep = pp_sepOpt !== undefined ? Caml_option.valFromOption(pp_sepOpt) : pp_print_cut;
...
function pp_print_option($staropt$star, pp_v, ppf, v) {
  var none = $staropt$star !== undefined ? Caml_option.valFromOption($staropt$star) : (function (param, param$1) {
