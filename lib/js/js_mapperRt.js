...
function binarySearch(upper, id, array) {
  ...
  while(true) {
    ...
    var lower = _lower;
    if (lower >= upper$1) {
      throw Caml_exceptions.stacktrace(new Error("binarySearchAux"));
    }
...
function revSearchAssert(len, array, x) {
  ...
  while(true) {
    var i = _i;
    if (i >= len$1) {
      throw Caml_exceptions.stacktrace(new Error("File \"js_mapperRt.ml\", line 63, characters 4-10"));
    }
    var match = xs[i];
    if (match[1] === k) {
      return match[0];
...
function fromIntAssert(len, xs, $$enum) {
  ...
  while(true) {
    var i = _i;
    if (i >= len$1) {
      throw Caml_exceptions.stacktrace(new Error("File \"js_mapperRt.ml\", line 87, characters 4-10"));
    }
    var k = xs$1[i];
    if (k === $$enum$1) {
      return i;
