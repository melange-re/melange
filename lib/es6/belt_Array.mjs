...
import * as Js_math from "./js_math.mjs";
...
function slice(a, offset, len) {
  ...
  var result$2 = new Array(copyLength);
  for(var i = 0; i < copyLength; ++i){
    result$2[i] = a[ofs + i | 0];
  }
  return result$2;
}
...
function sliceToEnd(a, offset) {
  ...
  var ofs = offset < 0 ? Caml_primitive.caml_int_max(lena + offset | 0, 0) : offset;
  var len = lena - ofs | 0;
  var result$3 = new Array(len);
  for(var i = 0; i < len; ++i){
    result$3[i] = a[ofs + i | 0];
  }
  return result$3;
}
...
function keepMapU(a, f) {
  ...
  for(var i = 0; i < l; ++i){
    ...
    var v$1 = f(v);
    if (v$1 !== undefined) {
      r$3[j] = Caml_option.valFromOption(v$1);
      j = j + 1 | 0;
    }
...
function unzip(a) {
  var l = a.length;
  var a1$1 = new Array(l);
  var a2$1 = new Array(l);
  for(var i = 0; i < l; ++i){
    var match = a[i];
    a1$1[i] = match[0];
    a2$1[i] = match[1];
