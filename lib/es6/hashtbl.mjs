...
function create(randomOpt, initial_size) {
  var random = randomOpt !== undefined ? Caml_option.valFromOption(randomOpt) : randomized[0];
...
function reset(h) {
  ...
  if ((h.length | 0) < 4 || len === Stdlib__no_aliases.abs(h.initial_size)) {
    ...
    h.size = 0;
    h.data = Caml_array.caml_make_vect(Stdlib__no_aliases.abs(h.initial_size), /* Empty */0);
    return ;
  }
}
...
function rebuild(randomOpt, h) {
  ...
  insert_all_buckets((function (param) {
          return key_index(h$prime, param);
        }), false, h.data, h$prime.data);
  return h$prime;
}
