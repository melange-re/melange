...
import * as Caml_hash from "./caml_hash.mjs";
...
function equal(x, y) {
  return Caml_primitive.caml_float_compare(x, y) === 0;
}
...
function make(n, v) {
  var result = Caml_array.caml_make_float_vect(n);
...
function init(l, f) {
  ...
  var res = Caml_array.caml_make_float_vect(l);
  for(var i = 0; i < l; ++i){
    res[i] = Curry._1(f, i);
  }
  return res;
...
function append(a1, a2) {
  var l1 = a1.length;
  var l2 = a2.length;
  var result = Caml_array.caml_make_float_vect(l1 + l2 | 0);
...
function concat(l) {
  ...
  var result = Caml_array.caml_make_float_vect(len);
...
function copy(a) {
  var l = a.length;
  var result = Caml_array.caml_make_float_vect(l);
...
function of_list(l) {
  var result = Caml_array.caml_make_float_vect(List.length(l));
  var _i = 0;
  var _l = l;
  while(true) {
    var l$1 = _l;
...
function map(f, a) {
  var l = a.length;
  var r = Caml_array.caml_make_float_vect(l);
  for(var i = 0; i < l; ++i){
    r[i] = Curry._1(f, a[i]);
  }
  return r;
...
function map2(f, a, b) {
  ...
  var r = Caml_array.caml_make_float_vect(la);
  for(var i = 0; i < la; ++i){
...
function mem(x, a) {
  ...
  while(true) {
    ...
    if (Caml_primitive.caml_float_compare(a[i], x) === 0) {
      return true;
    }
    _i = i + 1 | 0;
    continue ;
...
function stable_sort(cmp, a) {
  ...
  var l1 = l / 2 | 0;
  var l2 = l - l1 | 0;
  var t = Caml_array.caml_make_float_vect(l2);
  sortto(l1, t, 0, l2);
  sortto(0, a, l2, l1);
...
function of_seq(i) {
  ...
  var a = Caml_array.caml_make_float_vect(len);
  var _i = len - 1 | 0;
  var _param = l;
  while(true) {
    var param = _param;
...
var Array_create = Caml_array.caml_make_float_vect;
