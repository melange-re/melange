...
function diff(a, b) {
  ...
  if (cmp(tmp$1[sizea - 1 | 0], tmp$1[sizea]) < 0 || cmp(tmp$1[totalSize - 1 | 0], tmp$1[0]) < 0) {
    return {
            cmp: cmp,
            data: Belt_internalAVLset.copy(dataa)
          };
  ...
  var k = Belt_SortArray.diffU(tmp$1, 0, sizea, tmp$1, sizea, sizeb, tmp2$1, 0, cmp);
  return {
          cmp: cmp,
...
function union(a, b) {
  ...
  if (cmp(tmp$2[sizea - 1 | 0], tmp$2[sizea]) < 0) {
    return {
            cmp: cmp,
  ...
  var k = Belt_SortArray.unionU(tmp$2, 0, sizea, tmp$2, sizea, sizeb, tmp2$2, 0, cmp);
  return {
          cmp: cmp,
