...
let extract_ppx (map : json_map) (field : string) ~(cwd : string) :
  ...
  | Some (Arr { content }) ->
      ...
      Ext_array.fold_left content empty (fun { ppxlib; ppx_files } x ->
          ...
          if checked then { ppxlib; ppx_files = ppx_files @ [ ppx ] }
          else { ppxlib = ppxlib @ [ ppx ]; ppx_files })
  | Some config -> Bsb_exception.config_error config (field ^ " expect an array")
...
let rec interpret_json ~(package_kind : Bsb_package_kind.t) ~per_proj_dir :
    Bsb_config_types.t =
  ...
  | Obj { map } -> (
      ...
      let bs_dependencies =
        extract_dependencies
          ~package_kind:(Bsb_package_kind.Dependency package_specs) map
          per_proj_dir Bsb_build_schemas.bs_dependencies
      in
      let bs_dev_dependencies =
        match package_kind with
        | Toplevel ->
            extract_dependencies ~package_kind:(Dependency package_specs) map
              per_proj_dir Bsb_build_schemas.bs_dev_dependencies
        | Dependency _ -> []
      in
      ...
      | Some sources ->
          ...
          let groups =
            Bsb_parse_sources.scan ~ignored_dirs:(extract_ignored_dirs map)
              ~package_kind ~root:per_proj_dir ~cut_generators ~namespace
              sources
          in

          (match map.?(Bsb_build_schemas.bs_external_includes) with
          | Some _ ->
              Bsb_log.warn
                "@{<warning>Warning(%s):@} `%s` is not supported in Melange \
                 and will be ignored@."
      ...
      Ext_list.map (Bsb_build_util.get_list_string s) (fun s ->
          ...
          let dirs =
            Ext_list.filter_map files
              (fun ({ Bsb_file_groups.dir; is_dev; _ } as group) ->
                if (not is_dev) && not (Bsb_file_groups.is_empty group) then
                  Some (dep.package_path // dir)
                else None)
          in
          let install_dirs =
            Ext_list.filter_map files (fun { Bsb_file_groups.dir; is_dev; _ } ->
                if not is_dev then
                  Some
                    {
