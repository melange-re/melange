...
function is_val(th) {
  const match = Stdlib__Atomic.get(th);
  switch (match.TAG) {
    case /* Thunk */ 0 :
    case /* Forcing */ 1 :
      return false;
...
function force(th) {
  while (true) {
    const mut = Stdlib__Atomic.get(th);
    switch (mut.TAG) {
      case /* Thunk */ 0 :
