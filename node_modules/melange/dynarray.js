...
const CamlinternalOO = require("./camlinternalOO.js");
...
function unsafe_nocopy_to_array(a, dummy) {
  const n = a.length;
  if (n === 0 || Caml_array.get(a, 0).TAG !== Stdlib__Obj.double_tag) {
    for (let i = 0; i < n; ++i) {
      if (a[i] === dummy) {
...
function Dummy_of_val(prim) {
  return prim;
}

function Dummy_of_dummy(prim) {
  return prim;
}

function Dummy_unsafe_get(prim) {
  return prim;
}
...
const Dummy_Array = {
  ...
  Dummy_found$extension: Dummy_found,
  unsafe_nocopy_to_array,
  blit_array,
  blit,
  prefix,
  extend,
...
function index_out_of_bounds(fname, i, length) {
  if (length === 0) {
...
function negative_length_requested(fname, n) {
  return Curry._2(Stdlib__Printf.ksprintf(Stdlib.invalid_arg, {
    TAG: /* Format */ 0,
    _0: {
      TAG: /* String_literal */ 11,
...
function negative_capacity_requested(fname, n) {
  return Curry._2(Stdlib__Printf.ksprintf(Stdlib.invalid_arg, {
    TAG: /* Format */ 0,
    _0: {
      TAG: /* String_literal */ 11,
...
function requested_length_out_of_bounds(fname, requested_length) {
  return Curry._3(Stdlib__Printf.ksprintf(Stdlib.invalid_arg, {
    TAG: /* Format */ 0,
    _0: {
      TAG: /* String_literal */ 11,
  ...
  }), fname, requested_length, Stdlib__Sys.max_array_length);
}
...
function length_change_during_iteration(fname, expected, observed) {
  return Curry._3(Stdlib__Printf.ksprintf(Stdlib.invalid_arg, {
    TAG: /* Format */ 0,
    _0: {
      TAG: /* String_literal */ 11,
...
function unexpected_empty_element(fname, i, length) {
  if (i < length) {
    return missing_element(i, length);
  } else {
    return index_out_of_bounds(fname, i, length);
  }
}

function empty_dynarray(fname) {
...
function check_same_length(fname, a, length) {
  const length_a = a.length;
  if (length !== length_a) {
    return length_change_during_iteration(fname, length, length_a);
  }
...
function make$1(n, x) {
  ...
  const arr = Dummy_Array.make(n, x, dummy);
  return {
    length: n,
    arr,
    dummy
...
function init$1(n, f) {
  ...
  const dummy = global_dummy._0;
  const arr = Dummy_Array.init(n, f, dummy);
  return {
    length: n,
    arr,
    dummy
...
function set(a, i, x) {
  ...
  if (i >= length) {
    ...
    return Caml_array.set(arr, i, Dummy_of_val(x));
  }
}
...
function copy(param) {
  ...
  const arr$1 = Dummy_Array.prefix(arr, length);
  return {
    length,
    arr: arr$1,
    dummy: param.dummy
...
function find_last(a) {
  ...
  const i = length - 1 | 0;
  const v = arr[i];
  return Caml_option.some(v === a.dummy ? missing_element(i, length) : Dummy_unsafe_get(v));
}
...
function pop_last(a) {
  ...
  const v = arr[last];
  const v$1 = v === dummy ? missing_element(last, length) : Dummy_unsafe_get(v);
  arr[last] = Dummy_of_dummy(dummy);
  a.length = last;
  return v$1;
}
...
function remove_last(a) {
  ...
  if (last >= 0) {
    a.length = last;
    return Caml_array.set(a.arr, last, Dummy_of_dummy(a.dummy));
  }
...
function truncate(a, n) {
  ...
  if (length <= n) {
    ...
    a.length = n;
    return Stdlib__Array.fill(arr, n, length - n | 0, Dummy_of_dummy(a.dummy));
  }
}
...
function ensure_capacity(a, capacity_request) {
  ...
  const new_arr = Dummy_Array.extend(arr, a.length, a.dummy, new_capacity);
  a.arr = new_arr;
...
function fit_capacity(a) {
  if (a.arr.length === a.length) {
    ...
    a.arr = Dummy_Array.prefix(a.arr, a.length);
    return;
  }
}
...
function set_capacity(a, n) {
  ...
  if (n < cur_capacity) {
    ...
    a.arr = Dummy_Array.prefix(arr, n);
    return;
  } else if (n > cur_capacity) {
    a.arr = Dummy_Array.extend(arr, a.length, a.dummy, n);
    return;
  } else {
    return;
  }
...
function add_last(a, x) {
  const length = a.length;
  const arr = a.arr;
  if (length >= arr.length ? false : (a.length = length + 1 | 0, arr[length] = Dummy_of_val(x), true)) {
    return;
  }
  while (true) {
    ...
    const arr$1 = a.arr;
    if (length$1 >= arr$1.length ? false : (a.length = length$1 + 1 | 0, arr$1[length$1] = Dummy_of_val(x), true)) {
      return;
    }
    continue;
  };
...
function blit_assume_room(src, src_pos, src_length, dst, dst_pos, dst_length, blit_length) {
  ...
  try {
    return Dummy_Array.blit(src_arr, src.dummy, src_pos, dst_arr, dst.dummy, dst_pos, blit_length);
  }
  catch (raw_i){
    const i = Caml_js_exceptions.internalToOCamlException(raw_i);
    if (i.MEL_EXN_ID === Dummy_Array.Dummy_found$extension) {
      return missing_element(i._1, src_length);
    }
    throw i;
  }
}
...
function append_array_if_room(a, b) {
  ...
  if ((length_a + length_b | 0) > arr.length) {
    ...
    Dummy_Array.blit_array(b, 0, arr, length_a, length_b);
    return true;
  }
}
...
function iter_(fname, f, a) {
  ...
  for (let i = 0; i < length; ++i) {
    const v = arr[i];
    Curry._1(f, v === dummy ? missing_element(i, length) : Dummy_unsafe_get(v));
  }
...
function iteri(f, a) {
  ...
  for (let i = 0; i < length; ++i) {
    const v = arr[i];
    Curry._2(f, i, v === dummy ? missing_element(i, length) : Dummy_unsafe_get(v));
  }
  check_same_length("iteri", a, length);
}

function rev_iter(f, a) {
  const length = a.length;
  const arr = a.arr;
  const dummy = a.dummy;
  const capacity = arr.length;
...
function map(f, a) {
  ...
  const arr_out = Caml_array.make(length, Dummy_of_dummy(dummy));
  for (let i = 0; i < length; ++i) {
    const v = arr_in[i];
    arr_out[i] = Dummy_of_val(Curry._1(f, v === dummy ? missing_element(i, length) : Dummy_unsafe_get(v)));
  }
  const res = {
    length,
    arr: arr_out,
...
function mapi(f, a) {
  ...
  for (let i = 0; i < length; ++i) {
    const v = arr_in[i];
    arr_out[i] = Dummy_of_val(Curry._2(f, i, v === dummy ? missing_element(i, length) : Dummy_unsafe_get(v)));
  }
  const res = {
    length,
    arr: arr_out,
...
function fold_left(f, acc, a) {
  ...
  for (let i = 0; i < length; ++i) {
    const v = arr[i];
    const v$1 = v === dummy ? missing_element(i, length) : Dummy_unsafe_get(v);
    r = Curry._2(f, r, v$1);
  }
...
function exists(p, a) {
  ...
  const loop = function (p, arr, dummy, _i, length) {
    while (true) {
      ...
      const v = arr[i];
      if (Curry._1(p, v === dummy ? missing_element(i, length) : Dummy_unsafe_get(v))) {
        return true;
      }
      _i = i + 1 | 0;
      continue;
...
function exists2(p, a1, a2) {
  ...
  const loop = function (p, arr1, dummy1, arr2, dummy2, _i, length) {
    while (true) {
      ...
      const v = arr1[i];
      const v$1 = arr2[i];
      if (Curry._2(p, v === dummy1 ? missing_element(i, length) : Dummy_unsafe_get(v), v$1 === dummy2 ? missing_element(i, length) : Dummy_unsafe_get(v$1))) {
        return true;
      }
      _i = i + 1 | 0;
      continue;
...
function mem(x, a) {
  ...
  const loop = function (_i) {
    while (true) {
      ...
      const v = arr[i];
      if (Caml_obj.caml_equal(v === dummy ? missing_element(i, length) : Dummy_unsafe_get(v), x)) {
        return true;
      }
      _i = i + 1 | 0;
      continue;
...
function memq(x, a) {
  ...
  const loop = function (_i) {
    while (true) {
      ...
      const v = arr[i];
      if ((
          v === dummy ? missing_element(i, length) : Dummy_unsafe_get(v)
        ) === x) {
        return true;
      }
      _i = i + 1 | 0;
...
function find_opt(p, a) {
  ...
  const loop = function (_i) {
    while (true) {
      ...
      const v = arr[i];
      const x = v === dummy ? missing_element(i, length) : Dummy_unsafe_get(v);
      if (Curry._1(p, x)) {
        return Caml_option.some(x);
      }
      _i = i + 1 | 0;
...
function find_map(p, a) {
  ...
  const loop = function (_i) {
    while (true) {
      ...
      const v = arr[i];
      const r = Curry._1(p, v === dummy ? missing_element(i, length) : Dummy_unsafe_get(v));
      if (r !== undefined) {
        return r;
      }
      _i = i + 1 | 0;
...
function find_mapi(p, a) {
  ...
  const loop = function (_i) {
    while (true) {
      ...
      const v = arr[i];
      const r = Curry._2(p, i, v === dummy ? missing_element(i, length) : Dummy_unsafe_get(v));
      if (r !== undefined) {
        return r;
      }
      _i = i + 1 | 0;
...
function equal(eq, a1, a2) {
  ...
  const loop = function (_i) {
    while (true) {
      ...
      const v = arr1[i];
      const v$1 = arr2[i];
      if (!Curry._2(eq, v === dum1 ? missing_element(i, length) : Dummy_unsafe_get(v), v$1 === dum2 ? missing_element(i, length) : Dummy_unsafe_get(v$1))) {
        return false;
      }
      _i = i + 1 | 0;
      continue;
...
function compare(cmp, a1, a2) {
  ...
  const loop = function (_i) {
    while (true) {
      ...
      const v = arr1[i];
      const v$1 = arr2[i];
      const c = Curry._2(cmp, v === dum1 ? missing_element(i, length) : Dummy_unsafe_get(v), v$1 === dum2 ? missing_element(i, length) : Dummy_unsafe_get(v$1));
      if (c !== 0) {
        return c;
      }
      _i = i + 1 | 0;
...
function of_array(a) {
  const length = a.length;
  const dummy = global_dummy._0;
  const arr = Dummy_Array.copy_from_array(a, dummy);
  return {
    length,
    arr,
    dummy
...
function of_list(li) {
  ...
  const length = a.length;
  const dummy = global_dummy._0;
  const arr = Dummy_Array.unsafe_nocopy_from_array(a, dummy);
  return {
    length,
    arr,
    dummy
...
function to_list(a) {
  ...
  for (let i = length - 1 | 0; i >= 0; --i) {
    const v = arr[i];
    l = {
      hd: v === dummy ? missing_element(i, length) : Dummy_unsafe_get(v),
      tl: l
    };
  }
...
function unsafe_to_iarray(capacity, f) {
  ...
  const values = length === capacity$1 ? arr : Dummy_Array.prefix(arr, length);
  let tmp;
  try {
    tmp = Dummy_Array.unsafe_nocopy_to_array(values, a.dummy);
  }
  catch (raw_i){
    const i = Caml_js_exceptions.internalToOCamlException(raw_i);
    if (i.MEL_EXN_ID === Dummy_Array.Dummy_found$extension) {
      tmp = missing_element(i._1, length);
    } else {
      throw i;
    }
  }
