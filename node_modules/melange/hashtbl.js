...
const Stdlib__Domain = require("./domain.js");
const Stdlib__Int = require("./int.js");
const Stdlib__Random = require("./random.js");
...
function create(randomOpt, initial_size) {
  const random = randomOpt !== undefined ? randomOpt : randomized.v;
...
function insert_all_buckets(indexfun, inplace, odata, ndata) {
  ...
  const insert_bucket = function (_cell) {
    while (true) {
      const cell = _cell;
      if (/* tag */ typeof cell !== "object" && typeof cell !== "function") {
        return;
      }
      const key = cell.key;
      ...
      if (/* tag */ typeof tail !== "object" && typeof tail !== "function") {
...
function filter_map_inplace_bucket(f, h, i, _prec, _slot) {
  while (true) {
    const slot = _slot;
    const prec = _prec;
    if (/* tag */ typeof slot !== "object" && typeof slot !== "function") {
      if (/* tag */ typeof prec !== "object" && typeof prec !== "function") {
...
function to_seq(tbl) {
  ...
  const aux = function (_i, _buck, _param) {
    while (true) {
      const buck = _buck;
      const i = _i;
      if (/* tag */ typeof buck !== "object" && typeof buck !== "function") {
...
function MakeSeeded(H) {
  ...
  const remove_bucket = function (h, i, key, _prec, _bucket) {
    while (true) {
      const bucket = _bucket;
      const prec = _prec;
      if (/* tag */ typeof bucket !== "object" && typeof bucket !== "function") {
        return bucket;
      }
      const k = bucket.key;
  ...
  const find_and_remove = function (h, key) {
    const i = key_index(h, key);
    const bucket = remove_bucket(h, i, key, /* Empty */ 0, Caml_array.get(h.data, i));
    if (/* tag */ typeof bucket !== "object" && typeof bucket !== "function") {
      return;
    } else {
      return Caml_option.some(bucket.data);
    }
  };
  const remove = function (h, key) {
    const i = key_index(h, key);
    remove_bucket(h, i, key, /* Empty */ 0, Caml_array.get(h.data, i));
  };
  const find = function (h, key) {
    ...
    if (/* tag */ typeof next1 !== "object" && typeof next1 !== "function") {
      throw new Caml_js_exceptions.MelangeError(Stdlib.Not_found, {
          MEL_EXN_ID: Stdlib.Not_found
        });
    }
    ...
    if (/* tag */ typeof next2 !== "object" && typeof next2 !== "function") {
      throw new Caml_js_exceptions.MelangeError(Stdlib.Not_found, {
          MEL_EXN_ID: Stdlib.Not_found
        });
    }
  ...
  const retrieve_bucket = function (key, _bucket) {
    while (true) {
      const bucket = _bucket;
      if (/* tag */ typeof bucket !== "object" && typeof bucket !== "function") {
        return bucket;
  ...
  const replace_bucket = function (h, key, i, l, data, slot) {
    if (/* tag */ typeof slot !== "object" && typeof slot !== "function") {
  ...
  const find_and_replace = function (h, key, data) {
    const i = key_index(h, key);
    const l = Caml_array.get(h.data, i);
    const bucket = retrieve_bucket(key, l);
    let old_data;
    old_data = /* tag */ typeof bucket !== "object" && typeof bucket !== "function" ? undefined : Caml_option.some(bucket.data);
...
function remove_bucket(h, i, key, _prec, _bucket) {
  while (true) {
    const bucket = _bucket;
    const prec = _prec;
    if (/* tag */ typeof bucket !== "object" && typeof bucket !== "function") {
...
function find_and_remove(h, key) {
  const i = key_index(h, key);
  const bucket = remove_bucket(h, i, key, /* Empty */ 0, Caml_array.get(h.data, i));
  if (/* tag */ typeof bucket !== "object" && typeof bucket !== "function") {
    return;
...
function retrieve_bucket(key, _bucket) {
  while (true) {
    const bucket = _bucket;
    if (/* tag */ typeof bucket !== "object" && typeof bucket !== "function") {
      return bucket;
...
function replace_bucket(h, key, i, l, data, bucket) {
  if (/* tag */ typeof bucket !== "object" && typeof bucket !== "function") {
    Caml_array.set(h.data, i, {
      TAG: /* Cons */ 0,
      key: key,
