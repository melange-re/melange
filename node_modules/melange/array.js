...
function unsafe_stable_sort_sub(cmp, a, init_ofs, init_len) {
  ...
  const t = Caml_array.make(l2, Caml_array.get(a, init_ofs));
  sortto(init_ofs + l1 | 0, t, 0, l2);
  sortto(init_ofs, a, init_ofs + l2 | 0, l1);
  merge(init_ofs + l2 | 0, l1, t, 0, l2, a, init_ofs);
}

function stable_sort_sub(cmp, a, ofs, len) {
  if (ofs < 0 || len < 0 || ofs > (a.length - len | 0)) {
    throw new Caml_js_exceptions.MelangeError("Invalid_argument", {
        MEL_EXN_ID: "Invalid_argument",
  ...
  unsafe_stable_sort_sub(cmp, a, ofs, len);
}
...
function of_seq(i) {
  ...
  while (true) {
    ...
    const rev = _rev;
    const len = _len;
    const match = Curry._1(i$1, undefined);
    if (/* tag */ typeof match !== "object" && typeof match !== "function") {
      if (!rev) {
        return [];
      }
      const a = Caml_array.make(len, rev.hd);
      let _i$1 = len - 2 | 0;
      let _param = rev.tl;
      while (true) {
        const param = _param;
