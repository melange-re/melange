...
function set_size(state, break_hint) {
...
function pp_enqueue_break(state, token) {
  Stdlib__Queue.add(token, state.pp_queue);
  set_size(state, true);
  state.pp_right_total = state.pp_right_total + token.length | 0;
}

function scan_push(state, break_hint, token) {
  if (break_hint) {
    pp_enqueue_break(state, token);
  } else {
...
function lfprintf(ppf, param, args) {
  CamlinternalFormat.make_lprintf((function (param) {
    return output_acc(ppf, param);
  }), /* End_of_acc */ 0, param._0, args);
}

function lprintf(fmt, args) {
  lfprintf(Stdlib__Domain.DLS.get(std_formatter_key), fmt, args);
}

function leprintf(fmt, args) {
  lfprintf(Stdlib__Domain.DLS.get(err_formatter_key), fmt, args);
}

function lasprintf(param, args) {
  const b = Stdlib__Buffer.create(512);
  const ppf = formatter_of_buffer(b);
  const k = function (acc) {
    output_acc(ppf, acc);
    return flush_buffer_formatter(b, ppf);
  };
  return CamlinternalFormat.make_lprintf(k, /* End_of_acc */ 0, param._0, args);
}

function ldprintf(fmt, args, ppf) {
  lfprintf(ppf, fmt, args);
}
...
const Args = CamlinternalFormat.Args;
...
module.exports = {
  ...
  get_ellipsis_text,
  String_tag$extension: String_tag,
  String_tag,
  pp_open_stag,
  open_stag,
  pp_close_stag,
