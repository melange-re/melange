...
function to_string_default(x) {
  if (x.MEL_EXN_ID === Stdlib.Out_of_memory$extension) {
    return "Out of memory";
  }
  if (x.MEL_EXN_ID === Stdlib.Stack_overflow$extension) {
    return "Stack overflow";
  }
  if (x.MEL_EXN_ID === Stdlib.Match_failure$extension) {
    const match = x._1;
    const $$char = match[2];
    return Curry._5(Stdlib__Printf.sprintf(locfmt), match[0], match[1], $$char, $$char + 5 | 0, "Pattern matching failed");
  }
  if (x.MEL_EXN_ID === Stdlib.Assert_failure$extension) {
    const match$1 = x._1;
  ...
  if (x.MEL_EXN_ID === Stdlib.Undefined_recursive_module$extension) {
    const match$2 = x._1;
