...
function is_implicit(n) {
  if (is_relative(n) && !Stdlib__String.starts_with("./", n)) {
    return !Stdlib__String.starts_with("../", n);
  } else {
    return false;
  }
}
...
function is_implicit$1(n) {
  if (is_relative$1(n) && !Stdlib__String.starts_with("./", n) && !Stdlib__String.starts_with(".\\", n) && !Stdlib__String.starts_with("../", n)) {
    return !Stdlib__String.starts_with("..\\", n);
  } else {
    return false;
  }
...
function check_suffix$1(filename, suffix) {
  ...
  return Caml_bytes.bytes_to_string(Stdlib__Bytes.lowercase_ascii(Caml_bytes.bytes_of_string(r))) === Caml_bytes.bytes_to_string(Stdlib__Bytes.lowercase_ascii(Caml_bytes.bytes_of_string(suffix)));
}
...
function quote_cmd_filename(f) {
  ...
  if (Stdlib__Bytes.exists(f$3, Caml_bytes.bytes_of_string(f$1))) {
...
function chop_suffix_opt(suffix, name) {
  if (Curry._2(check_suffix$2, name, suffix)) {
    return Stdlib__String.sub(name, 0, name.length - suffix.length | 0);
  }
...
const current_temp_dir_name = Stdlib__Domain.DLS.new_key(Stdlib__Fun.id, (function (param) {
...
function temp_file(temp_dirOpt, prefix, suffix) {
  ...
  while (true) {
    ...
    try {
      ...
      if (e.MEL_EXN_ID === Stdlib.Sys_error$extension) {
        if (counter >= 20) {
          throw e;
        }
...
module.exports = {
  ...
  dir_sep,
  concat,
  is_relative: is_relative$2,
  is_implicit: is_implicit$2,
  check_suffix: check_suffix$2,
  chop_suffix,
  chop_suffix_opt,
  extension,
  remove_extension,
