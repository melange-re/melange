// Generated by Melange
'use strict';

const Curry = require("melange.js/curry.js");

function forEachU(s, f, action) {
  for (let i = s; i <= f; ++i) {
    action(i);
  }
}

function forEach(s, f, action) {
  forEachU(s, f, Curry.__1(action));
}

function everyByAux(_s, f, step, p) {
  while (true) {
    const s = _s;
    if (s > f) {
      return true;
    }
    if (!p(s)) {
      return false;
    }
    _s = s + step | 0;
    continue;
  };
}

function everyU(s, f, p) {
  return everyByAux(s, f, 1, p);
}

function every(s, f, p) {
  return everyU(s, f, Curry.__1(p));
}

function everyByU(s, f, step, p) {
  if (step > 0) {
    return everyByAux(s, f, step, p);
  } else {
    return true;
  }
}

function everyBy(s, f, step, p) {
  return everyByU(s, f, step, Curry.__1(p));
}

function someByAux(_s, f, step, p) {
  while (true) {
    const s = _s;
    if (s > f) {
      return false;
    }
    if (p(s)) {
      return true;
    }
    _s = s + step | 0;
    continue;
  };
}

function someU(s, f, p) {
  return someByAux(s, f, 1, p);
}

function some(s, f, p) {
  return someU(s, f, Curry.__1(p));
}

function someByU(s, f, step, p) {
  if (step > 0) {
    return someByAux(s, f, step, p);
  } else {
    return false;
  }
}

function someBy(s, f, step, p) {
  return someByU(s, f, step, Curry.__1(p));
}

module.exports = {
  forEachU,
  forEach,
  everyU,
  every,
  everyByU,
  everyBy,
  someU,
  some,
  someByU,
  someBy,
}
/* No side effect */
