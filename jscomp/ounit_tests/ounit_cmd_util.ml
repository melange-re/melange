...
type output = { stderr : string; stdout : string; exit_code : int }
...
let perform command args =
  ...
  let pid =
    Unix.create_process command args in_fd_read out_fd_write err_fd_write
  in
  ...
  *)
  ...
  let exit_code =
    ...
    | Unix.WSIGNALED _signal_number | Unix.WSTOPPED _signal_number -> 127
  in
...
let bsc_check_eval str = perform_bsc [| "-bs-eval"; str |]
