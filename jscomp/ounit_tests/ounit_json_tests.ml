  ...
  [
    ...
    "empty trails" >:: begin fun _ ->
      (OUnit.assert_raises Parse_error @@ fun _ ->
       try ignore @@ parse_json_from_string {| [,]|} with _ -> raise Parse_error);
      OUnit.assert_raises Parse_error @@ fun _ ->
      try ignore @@ parse_json_from_string {| {,}|} with _ -> raise Parse_error
    end;
    "two trails" >:: begin fun _ ->
      (OUnit.assert_raises Parse_error @@ fun _ ->
       try ignore @@ parse_json_from_string {| [1,2,,]|} with _ -> raise Parse_error);
      (OUnit.assert_raises Parse_error @@ fun _ ->
       try ignore @@ parse_json_from_string {| { "x": 3, ,}|} with _ -> raise Parse_error)
    end;

    "two trails fail" >:: begin fun _ ->
      (OUnit.assert_raises Parse_error @@ fun _ ->
       try ignore @@ parse_json_from_string {| { "x": 3, 2 ,}|} with _ -> raise Parse_error)
    end;
