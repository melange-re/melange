...
let constantString ~loc str = Ast_helper.Exp.constant ~loc (Pconst_string (str, Location.none, None))
...
let unerasableIgnore loc =
  { attr_name = { loc; txt = "warning" };
    attr_payload = PStr [ Str.eval (Exp.constant (Pconst_string ("-16", Location.none, None))) ];
    attr_loc = loc;
  }
    ...
    pval_type =
      recursivelyMakeNamedArgsForExternal namedArgListWithKeyAndRef
        (Typ.arrow nolabel
           ...
           propsType);
    pval_prim = [ "" ];
    pval_attributes = [ { attr_name = { txt = "bs.obj"; loc }; attr_payload = PStr []; attr_loc = loc } ];
    pval_loc = loc;
  }
  [@@raises Invalid_argument]
...
let jsxMapper () =
        ...
        let mapBinding binding =
          if hasAttrOnBinding binding then
            ...
            let _attr_loc, payload =
              ...
              | Some { attr_name = loc; attr_payload = payload; _ } -> (loc.loc, Some payload)
              | None -> (emptyLoc, None)
            in
  ...
  let expr mapper expression =
    ...
    | { pexp_desc = Pexp_apply (callExpression, callArguments); pexp_attributes } -> (
        let jsxAttribute, nonJSXAttributes =
          List.partition (fun {attr_name = attribute; _} -> attribute.txt = "JSX") pexp_attributes
        in
  ...
  let module_binding mapper module_binding =
    ...
    | None -> ()
    | Some name ->
        nestedModules := name :: !nestedModules);
