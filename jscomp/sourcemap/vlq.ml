...
module Stream = struct
  ...
  let of_string s = { s; len = String.length s; i = 0 }
  let peek t = if t.i >= t.len then None else Some t.s.[t.i]
  let junk t = if t.i < t.len then t.i <- t.i + 1
...
module Make (C : Config) = struct
  ...
  let vlq_signed_of_int value =
    if value < 0 then ((-value) lsl 1) + 1 else value lsl 1
  ...
  let decode =
    let rec helper (acc, shift) stream =
      let chr = try Stream.next stream with Stream.Failure -> raise Unexpected_eof in
...
module Base64 = Make (struct
  ...
  let char_of_digit digit =
    if 0 <= digit && digit < String.length base64 then base64.[digit]
    else failwith (Printf.sprintf "Must be between 0 and 63: %d" digit)

  let digit_of_char chr =
    try String.index base64 chr with Not_found -> raise (Invalid_base64 chr)
end)
