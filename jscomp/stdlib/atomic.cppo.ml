...
#ifndef MELANGE
  (* Atomic-location loads are not yet supported by the JavaScript backend. *)
  let update f t =
...
#if defined MELANGE || OCAML_VERSION >= (5,6,0)
module Array = struct
  type !'a t =
    'a array
...
#ifdef MELANGE
  (* JavaScript arrays need neither uniform allocation nor atomic locations. *)
  let check_array_bound t i =
  ...
  let unsafe_compare_and_set t i old new_ =
    if unsafe_get t i == old then begin
      unsafe_set t i new_;
      true
    end else false
#else
...
#ifdef MELANGE
  let rec unsafe_update f t i =
    let old = unsafe_get t i in
    let new_ = f old in
    if old == new_ || unsafe_compare_and_set t i old new_ then ()
    else unsafe_update f t i
#else
