...
let suites = Mt.[
  ...
  "hypotMany", (fun _ -> ApproxThreshold(0.001, 1.077, hypotMany [| 0.4; 0.6; 0.8 |]));
  ...
  "random", (fun _ -> Ok(let a = random () in a >= 0. && a < 1.));
  "random_int", (fun _ -> Ok(let a = random_int 1 3 in a >= 1 && a < 3));
