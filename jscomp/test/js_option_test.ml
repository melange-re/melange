...
let simpleEq = (fun [@u] (a : int) b -> a = b)
...
let option_suites = Mt.[
    ...
    "option_andThen_SomeSome", (fun _ ->
        Eq (true ,
            Js.Option.isSomeValue simpleEq 3 (Js.Option.andThen (fun [@u] a -> Some (a + 1)) (Some 2)))
      );
    "option_andThen_SomeNone", (fun _ ->
        Eq (false ,
            Js.Option.isSomeValue simpleEq 3 (Js.Option.andThen (fun [@u] _ -> None) (Some 2)))
      );
    "option_map_Some", (fun _ ->
        Eq (true ,
            Js.Option.isSomeValue simpleEq 3 (Js.Option.map (fun [@u] a -> a + 1) (Some 2)))
      );
    "option_map_None", (fun _ ->
        Eq (None , Js.Option.map (fun [@u] a -> a + 1) None)
      );
    ...
    "option_filter_Pass", (fun _ ->
        Eq (true ,
            Js.Option.isSomeValue simpleEq 2 (Js.Option.filter (fun [@u] a -> (a mod 2) = 0) (Some 2)))
      );
    "option_filter_Reject", (fun _ ->
        Eq (None , Js.Option.filter (fun [@u] a -> (a mod 3) = 0) (Some 2))
      );
    "option_filter_None", (fun _ ->
        Eq (None , Js.Option.filter (fun [@u] a -> (a mod 3) = 0) None)
      );
