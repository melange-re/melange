...
let suites :  Mt.pair_suites = Mt.[
    ...
    "fac_int64", (fun _ ->
     ...
     Ext_array_test.map2i (fun i a b -> Printf.sprintf "shift_left_cases %d" i, (fun _ -> Mt.Eq(a,b)) ) a b
     |>  Array.to_list)
                                 @
                                 ((let (a,b) = shift_right_tests in
                                   Ext_array_test.map2i (fun i a b -> Printf.sprintf "shift_right_cases %d" i, (fun _ -> Mt.Eq(a,b)) ) a b
                                   |>  Array.to_list))    @
                                 ((let (a,b) = shift_right_logical_suites in
                                   Ext_array_test.map2i (fun i a b -> Printf.sprintf "shift_right_logical_cases %d" i, (fun _ -> Mt.Eq(a,b)) ) a b
                                   |>  Array.to_list))
