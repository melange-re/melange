...
let match = bufferize(function (param) {
...
function peekch(param) {
  let ch = Curry._1(getch, undefined);
  Curry._1(ungetch, ch);
  return ch;
}
...
let glo = Stdlib__Bytes.make(4096, /* '\000' */0);

let gpos = {
  contents: 0
};

let s = Caml_bytes.caml_create_bytes(100);

function getq(param) {
  let c = Curry._1(getch, undefined);
    ...
    if (ch >= 14) {
      if (ch !== 32) {
        if (ch !== 47 || peekch(undefined) !== /* '*' */42) {
          ...
          while(true) {
            let match = Curry._1(getch, undefined);
            if (match !== 42) {
              _param$1 = undefined;
              continue ;
            }
...
function next(param) {
  ...
  if (c >= 48) {
    ...
    let ch = getq(undefined);
    let qt = Curry._1(getch, undefined);
    if (qt !== /* '\'' */39) {
...
let match$1 = bufferize(next);

let unnext = match$1[1];

let next$1 = match$1[0];
...
let obuf = Stdlib__Bytes.make(1048576, /* '\000' */0);
...
function binary(stk, lvl) {
  ...
  let foldtst = function (_loc) {
    while(true) {
      let loc = _loc;
      let o = Curry._1(next$1, undefined);
      if (o.TAG === /* Op */0) {
        if (lvlof(o._0) === lvl) {
          let loc$p = test(lvl - 8 | 0, loc);
          binary(stk, lvl - 1 | 0);
          _loc = loc$p;
          continue ;
        }
  ...
  switch (i.TAG | 0) {
    ...
    case /* Sym */3 :
        ...
        if (Stdlib__List.mem_assoc(i$1, stk)) {
          let l = Stdlib__List.assoc(i$1, stk);
          if (l <= -256) {
            throw new Caml_js_exceptions.MelangeError("Assert_failure", {
                      MEL_EXN_ID: "Assert_failure",
                      _1: [
          ...
          out(18616);
          let g = Caml_array.get(globs, i$1);
          let loc = opos.contents;
          le(64, g.loc);
      ...
      return function vars(_n, _stk) {
        while(true) {
          ...
          if (s.TAG === /* Sym */3) {
            ...
            let stk$p;
            if (g) {
              let glo = Caml_array.get(globs, s$1);
              if (glo.va >= 0) {
                throw new Caml_js_exceptions.MelangeError("Failure", {
                          MEL_EXN_ID: "Failure",
              ...
              let va = (gpos.contents + 232 | 0) + 4194304 | 0;
...
function stmt(brk, stk) {
  ...
  if (Caml_obj.caml_equal(t, tokwhile) || Caml_obj.caml_equal(t, tokfor)) {
    ...
    let ba = align.contents;
    let match;
    if (Caml_obj.caml_equal(t, tokwhile)) {
      let loc$2 = opos.contents;
      pexpr(stk);
      ...
      Curry._1(next$1, undefined);
      out(233);
      let bdy = opos.contents;
      le(32, 0);
      let itr = opos.contents;
      expr(stk);
      Curry._1(next$1, undefined);
      out(233);
...
function top(_param) {
  while(true) {
    ...
    if (f.TAG === /* Sym */3) {
      let f$1 = f._0;
      let g = Caml_array.get(globs, f$1);
      if (g.va >= 0) {
        throw new Caml_js_exceptions.MelangeError("Failure", {
                  MEL_EXN_ID: "Failure",
                  _1: "symbol defined twice"
      ...
      let emitargs = function (_regs, _n, _stk) {
        while(true) {
          let stk = _stk;
          let n = _n;
          let regs = _regs;
...
let elfhdr = Stdlib__Bytes.of_string(Stdlib__String.concat("", {
...
function elfgen(outf) {
  let entry = opos.contents;
  let main = addsym("main");
  let gmain = Caml_array.get(globs, main);
  out(1217084452);
  out(-1921768440);
  out(18616);
  le(64, gmain.loc);
  ...
  load(0, 60);
  out(3845);
  let off = 232 + gpos.contents | 0;
  let itr = function (f) {
    symitr(function (i, s) {
          let g = Caml_array.get(globs, i);
          if (g.va < 0 && g.loc !== 0) {
  ...
  let rel = opos.contents;
  let n$1 = {
    contents: 1
  };
  itr(function (param, param$1, l) {
        let genrel = function (_l) {
          while(true) {
            let l = _l;
            if (l === 0) {
              return ;
  ...
  let hash = opos.contents;
  let n$2 = ((rel - symtab | 0) / 24 | 0) - 1 | 0;
  le(32, 1);
  le(32, n$2 + 1 | 0);
  le(32, n$2 > 0 ? 1 : 0);
  for(let i = 1; i <= n$2; ++i){
...
function main(param) {
  ...
  let f = Caml_sys.caml_sys_argv(0).length < 2 ? "-blk" : Caml_array.get(Caml_sys.caml_sys_argv(0), 1);
  switch (f) {
    case "-blk" :
        let partial_arg_0 = {
          contents: 0
