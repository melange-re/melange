...
let CamlinternalFormatBasics = require("melange/camlinternalFormatBasics.js");
...
function $caret(s1, s2) {
  let l1 = s1.length;
  let l2 = s2.length;
  let s = Caml_bytes.caml_create_bytes(l1 + l2 | 0);
...
let stdin = Caml_io.stdin;

let stdout = Caml_io.stdout;

let stderr = Caml_io.stderr;
...
function unsafe_really_input(ic, s, _ofs, _len) {
  while(true) {
    ...
    let r = Caml_external_polyfill.resolve("caml_ml_input")(ic, s, ofs, len);
    if (r === 0) {
...
function really_input_string(ic, len) {
  let s = Caml_bytes.caml_create_bytes(len);
...
function input_line(chan) {
  ...
  while(true) {
    let len = _len;
    let accu = _accu;
    let n = Caml_external_polyfill.resolve("caml_ml_input_scan_line")(chan);
    if (n === 0) {
      if (accu) {
    ...
    if (n > 0) {
      let res = Caml_bytes.caml_create_bytes(n - 1 | 0);
    ...
    let beg = Caml_bytes.caml_create_bytes(-n | 0);
