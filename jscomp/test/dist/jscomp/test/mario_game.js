...
function setup_sprite(loopOpt, bbox_offsetOpt, bbox_sizeOpt, img_src, max_frames, max_ticks, frame_size, src_offset) {
  ...
  let bbox_size$1 = Caml_obj.caml_equal(bbox_size, [
        0,
        0
...
function make$2($staropt$star, $staropt$star$1, spawnable, context, param) {
  ...
  let dir = $staropt$star$1 !== undefined ? $staropt$star$1 : /* Left */0;
...
function update_player(player, keys, context) {
  ...
  let pl_typ = player.health <= 1 ? /* SmallM */1 : /* BigM */0;
    ...
    case /* Player */0 :
        ...
        switch (c2.TAG | 0) {
          ...
          case /* Block */3 :
              ...
              let updated_block = evolve_block(o2$3, context);
              let spawned_item = spawn_above(o1$3.dir, o2$3, t._0, context);
              collide_block(undefined, dir, o1$3);
              return [
                      spawned_item,
                      updated_block
          ...
          case /* Block */3 :
              ...
              if (dir >= 2) {
                if (t1 >= 3) {
                  ...
                  let updated_block$1 = evolve_block(o2$5, context);
                  let spawned_item$1 = spawn_above(o1$4.dir, o2$5, t2$3._0, context);
                  rev_dir(o1$4, t1, s1$3);
                  return [
                          updated_block$1,
                          spawned_item$1
...
function generate_coins(_block_coord) {
  while(true) {
    ...
    let place_coin = Stdlib__Random.$$int(2);
    if (!block_coord) {
      return /* [] */0;
    }
...
function choose_block_pattern(blockw, blockh, cbx, cby, prob) {
  ...
  let block_typ = Stdlib__Random.$$int(4);
  let stair_typ = Stdlib__Random.$$int(2);
  let life_block_chance = Stdlib__Random.$$int(5);
  ...
  switch (prob) {
    ...
    case 1 :
        let num_clouds = Stdlib__Random.$$int(5) + 5 | 0;
        if (cby < 5) {
...
function generate_enemies(blockw, blockh, _cbx, _cby, acc) {
  while(true) {
    ...
    let prob = Stdlib__Random.$$int(30);
...
function generate_block_enemies(_block_coord) {
  while(true) {
    let block_coord = _block_coord;
    let place_enemy = Stdlib__Random.$$int(20);
    let enemy_typ = Stdlib__Random.$$int(3);
    if (!block_coord) {
      return /* [] */0;
    }
    let t = block_coord.tl;
...
function generate_ground(blockw, blockh, _inc, _acc) {
  while(true) {
    ...
    if (inc > 10) {
      let skip = Stdlib__Random.$$int(10);
      let newacc = Stdlib.$at(acc, {
            hd: [
              4,
              [
                inc * 16,
...
function generate_helper(blockw, blockh, cx, cy, context) {
  ...
  let block_locations = Stdlib.$at(block_locs, ground_blocks);
  let all_blocks = Stdlib.$at(obj_converted_block_locs, obj_converted_ground_blocks);
  ...
  let converted_block_coin_locs = Stdlib.$at(converted_block_locs, coin_locs);
