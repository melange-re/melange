...
let wrap_int x =
  { show = (fun () -> ()); to_string = (fun () -> string_of_int x) }

let wrap_float x =
  { show = (fun () -> ()); to_string = (fun () -> string_of_float x) }
...
let test_classes () =
  let rec build_classes n acc =
    if n <= 0 then acc
      ...
      build_classes (pred n)
        ((new floating (float_of_int n) :> h) :: (new integer n :> h) :: acc)
  in
  let t1 = Sys.time () in
...
let test_records () =
  let rec build_records n acc =
    if n <= 0 then acc
      ...
      build_records (pred n) (wrap_float (float_of_int n) :: wrap_int n :: acc)
  in
  let t1 = Sys.time () in
...
let _ =
  let tci, tca = test_classes () and tri, tra = test_records () in
  Printf.printf
