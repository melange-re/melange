...
let string_of_module_id ~package_info ~output_info
  ...
  (* This may not be enough,
    ...
    *)
  | Runtime -> get_runtime_module_path ~package_info ~output_info dep_module_id
  | Ml -> (
      ...
      | Package_found dep_info, Package_found cur_pkg -> (
          ...
          in
          match
            Js_packages_info.same_package_by_name package_info dep_package_info
          with
          | true ->
              (* If this is the same package, we know all imports are relative. *)
...
let string_of_module_id =
  let fix_path_for_windows =
    let replace_backward_slash x =
      match String.index x '\\' with
      | _i -> String.map ~f:(function '\\' -> '/' | x -> x) x
      | exception Not_found -> x
    in
    if Sys.win32 || Sys.cygwin then replace_backward_slash else Fun.id
  in
  fun ~package_info ~output_info dep_module_id ~output_dir ->
    string_of_module_id ~package_info ~output_info dep_module_id ~output_dir
