...
let transform_under_supply n ap_info fn args =
  let extra_args = Ext_list.init n
      (fun _ ->   (Ident.create_local Literals.param)) in
      ...
      if from = to_ then
        ...
        | Lfunction{params = [param]; body} ->
          Lam.function_ ~arity:0
            ...
            ) ~loc (* could be only introduced by
                 {[ Pjs_fn_make 0 ]} <-
                 {[ fun [@bs] () -> .. ]}
              *)
        | _ ->
        ...
        | _ ->
          let arity = to_ in
          let extra_args = Ext_list.init to_  (fun _ -> Ident.create_local Literals.param ) in
          let wrapper, new_fn =
            match fn with
            | Lvar _
            | Lprim {primitive = Pfield (_,Fld_module _) ; args = [ Lglobal_module _ | Lvar _] ; _}  ->
          ...
          let cont =
            Lam.function_
              ...
              ~body:(
                ...
                Lam.apply (Lam.apply new_fn (Ext_list.map first_args  Lam.var) {ap_info with ap_status = App_infer_full} ) (Ext_list.map rest_args Lam.var) ap_info  )
              ~loc in
        ...
        *)
        begin match fn with
          ...
          | Lfunction
              {params; body; loc} (* TODO check arity = List.length params in debug mode *)
            ->
            let arity = to_ in
            ...
            Lam.function_
              ...
              ~body:(
                Lam.function_ ~arity:(from - to_) ~attr:Lam.default_fn_attr
                  ~params:extra_inner_args ~body:body ~loc)
              ~loc
          | _
            ->
                ...
                ~body:(
                  ...
                  Lam.function_ ~arity ~params:extra_inner_args  ~attr:Lam.default_fn_attr
                    ~body:(Lam.apply new_fn
                             ...
                             {ap_info with ap_status = App_infer_full} ) ~loc
                ) ~loc in
            begin match wrapper with
              | None -> cont
