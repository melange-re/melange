...
type t = { prim_name : string; lowering : lowering; effects : effects }

let make ?(effects = May_have_side_effects) prim_name lowering =
  { prim_name; lowering; effects }

let builtin ?effects prim_name builtin =
  make ?effects prim_name (Builtin builtin)

let runtime ?effects prim_name runtime_module runtime_name =
  builtin ?effects prim_name (Runtime_call (runtime_module, runtime_name))

let runtime_primitive ?effects prim_name runtime_module =
  runtime ?effects prim_name runtime_module Primitive_name
...
let of_name prim_name =
  ...
  | "caml_eq_float" -> builtin prim_name Float_equal
  | "caml_ge_float" -> builtin prim_name Float_greater_equal
  | "caml_gt_float" -> builtin prim_name Float_greater
  ...
  | "caml_int64_equal_null" -> builtin prim_name (Int64 Int64_equal_null)
  | "caml_int64_equal_undefined" ->
      builtin prim_name (Int64 Int64_equal_undefined)
  | "caml_int64_equal_nullable" ->
      builtin prim_name (Int64 Int64_equal_nullable)
  ...
  | "caml_string_lessequal" -> builtin prim_name (String_comparison Le)
  | "caml_string_lessthan" -> builtin prim_name (String_comparison Lt)
  | "caml_string_greaterequal" -> builtin prim_name (String_comparison Ge)
  | "caml_string_greaterthan" -> builtin prim_name (String_comparison Gt)
  | "caml_string_repeat" ->
      builtin ~effects:No_side_effects prim_name String_repeat
  | "caml_bool_notequal" -> builtin prim_name (Bool_comparison Ne)
  | "caml_bool_lessequal" -> builtin prim_name (Bool_comparison Le)
  | "caml_bool_lessthan" -> builtin prim_name (Bool_comparison Lt)
  | "caml_bool_greaterequal" -> builtin prim_name (Bool_comparison Ge)
  | "caml_bool_greaterthan" -> builtin prim_name (Bool_comparison Gt)
  | "caml_bool_equal" | "caml_bool_equal_null" | "caml_bool_equal_nullable"
  | "caml_bool_equal_undefined" ->
      builtin prim_name (Bool_comparison Eq)
  ...
  | "caml_notequal" ->
      builtin prim_name (Polymorphic_comparison Polymorphic_not_equal)
  | "caml_equal" -> builtin prim_name (Polymorphic_comparison Polymorphic_equal)
...
let returns_boolean t =
  match t.lowering with
  | Builtin (String_comparison (Eq | Ne)) -> true
