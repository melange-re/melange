...
and pp_one_case_clause :
      'a. _ -> (_ -> 'a -> unit) -> 'a * J.case_clause -> _ =
 fun cxt pp_cond
...
and expression_desc cxt ~(level:int) x : cxt  =
  ...
  | Fun (is_method, l, b, env, return_unit) ->  (* TODO: dump for comments *)
    pp_function ~return_unit ~is_method cxt ~fn_state:default_fn_exp_state  l b env
  (* TODO:
          ...
          | _, [] ->
            ...
            paren_group cxt 1 (fun _ ->
                ...
                | [{expression_desc = Fun (is_method, l,b,env, return_unit)}]
                  ->
                  pp_function ~return_unit ~is_method cxt ~fn_state:(No_name {single_arg = true})
                    l b env
                | _ ->
                  arguments cxt el
              )
  ...
  | Return e ->
    begin match e.expression_desc with
      | Fun (is_method,  l, b, env, return_unit) ->
        let cxt =
          pp_function ~return_unit ~is_method cxt ~fn_state:Is_return l b env in
        semi cxt; cxt
      | Undefined ->
        return_sp cxt;
        semi cxt;
