...
let ident cxt id = 
  update_scope cxt (Ext_pp_scope.ident cxt.scope cxt.pp id)
...
let pp_direction cxt (direction : J.for_direction) =
  ...
  | Up
  | Upto -> string cxt L.plus_plus
  | Downto -> string cxt L.minus_minus

let return_sp cxt =
    string cxt L.return ; space cxt

let bool cxt b =
  string cxt (if b then  L.true_ else L.false_)

let comma_sp cxt =
  comma cxt; space cxt
let comma_nl cxt =
  comma cxt; newline cxt
...
let formal_parameter_list cxt  l   =
    iter_lst cxt l ident comma_sp
(* IdentMap *)
(*
f/122 -->
  f/122 is in the map
...
*)
let rec
  try_optimize_curry cxt len function_id =
...
and  pp_function ~is_method
  ...
  | [ {statement_desc =
    ...
    let optimize  len ~p cxt v =
      if p then try_optimize_curry cxt len function_id
      else
        vident cxt v in
  ...
  | _  ->
    let set_env : Set_ident.t = (** identifiers will be printed cxtollowing*)
      match fn_state with
      | Is_return
      | No_name _ ->
    ...
    *)
    ...
    let param_body () : unit =
      if is_method then
        ...
        | [] -> assert false
        | this::arguments ->
          let cxt = paren_group cxt 1 (fun _ ->
          ...
          brace_vgroup cxt 1 (fun _ ->
              let cxt =
                ...
                else  pp_var_assign_this cxt this in
              function_body cxt b
            );
      else
        let cxt =
          paren_group cxt 1 (fun _ -> formal_parameter_list inner_cxt  l  ) in
        space cxt;
        brace_vgroup cxt 1 (fun _ -> function_body cxt b )
    in
      ...
      let handle lexical =
        ...
        then
          ...
          *)
          ...
          | _ -> semi cxt (* has binding, a statement *)  in
      handle
        (match fn_state with
    ...
    let cxt =
      group cxt 1  (fun _ ->
          group cxt 1 (fun _ ->
              ...
              pp_cond cxt switch_case; (* could be integer or string *)
              space cxt;
              string cxt L.colon  );
          group cxt 1 (fun _ ->
              let cxt =
                ...
                | _ ->
                  newline cxt;
                  statement_list false cxt switch_body
              in
...
and expression_desc cxt ~(level:int) x : cxt  =
  match x with
  | Null ->
    string cxt L.null; cxt
  | Undefined ->
    string cxt L.undefined; cxt
  | Var v ->
    vident cxt v
  | Bool b ->
    bool cxt b ; cxt
  ...
  | Fun (is_method, l, b, env) ->  (* TODO: dump for comments *)
    pp_function ~is_method cxt ~fn_state:default_fn_exp_state  l b env
  (* TODO:
  ...
  *)
          ...
          | _, [] ->
            ...
            paren_group cxt 1 (fun _ ->
                ...
                | [{expression_desc = Fun (is_method, l,b,env)}]
                  ->
                  pp_function ~is_method cxt ~fn_state:(No_name {single_arg = true})
                    l b env
                | _ ->
                  arguments cxt el
              )
          ...
          | _ , _ ->
            ...
            if 1 <= len && len <= 8 then
              ...
              begin
                ...
                paren_group cxt 1
                  (fun _ -> arguments cxt [ e ; E.array Mutable el])
              end))
  | FlatCall(e,el) ->
    group cxt 1 (fun _ ->
        let cxt = expression ~level:15 cxt e in
  ...
  | Number v ->
    ...
    let need_paren =
      ...
      then level > 13  (* Negative numbers may need to be parenthesized. *)
           ...
           && s.[0] <> 'N' (* NaN *) in
    let action =
      fun _ -> string cxt s  in
    (
      if need_paren
      then paren cxt action
      else action ()
    );
    cxt
    ...
    *)
    ...
    (match delta, op with
     ...
     | {expression_desc = Number (Int { i =  -1l; _})}, Minus
       ->
       string cxt L.plusplus;
       space cxt;
       expression ~level:13 cxt lhs (* Static index level is 15*)
     | {expression_desc = Number (Int { i =  -1l; _})}, Plus
     | {expression_desc = Number (Int { i =  1l; _})}, Minus
       ->
       string cxt L.minusminus;
       space cxt;
       expression ~level:13 cxt lhs

     | _, _ ->
       let cxt = expression ~level:13 cxt lhs in
       space cxt;
       string cxt (if op = Plus then "+=" else "-=");
       space cxt;
       expression ~level:13 cxt delta)
  ...
  | Caml_block(el,_, _, Blk_module fields) ->
      expression_desc cxt ~level (Object (
  ...
  | String_index (e,p)
    ...
    cond_paren_group cxt (level > 15) 1 (fun _ ->
        group cxt 1  (fun _ ->
        let cxt = expression ~level:15 cxt e in
        bracket_group cxt 1 (fun _ ->
        expression ~level:0 cxt p )))
  | Static_index (e, s,_) ->
    cond_paren_group cxt (level > 15) 1 (fun _ ->
        let cxt = expression ~level:15 cxt e in
  ...
  | Object lst ->
    ...
    cond_paren_group cxt (level > 1 ) 1 (fun _ ->
        if lst = [] then begin string cxt "{}" ; cxt end else
          brace_vgroup cxt 1 (fun _ ->
...
and variable_declaration top cxt
...
and pp_comment cxt comment =
  if String.length comment > 0 then
    begin
      string cxt "/* "; string cxt comment ; string cxt " */"
    end

and pp_comment_option cxt comment  =
  match comment with
  | None -> ()
  | Some x -> pp_comment cxt x

(* and pp_loc_option f loc =  *)
and statement top cxt
...
and statement_desc top cxt (s : J.statement_desc) : cxt =
  match s with
  | Block [] ->
    ipp_comment cxt  L.empty_block; (* debugging*)
    cxt
    ...
    (
      ...
      | _ ->
        let cxt =
          (
            if exp_need_paren  e
            then paren_group cxt 1
            else group cxt 0
          ) (fun _ -> expression ~level:0 cxt e ) in
        semi cxt;
        cxt)
  | Block b -> (* No braces needed here *)
    ipp_comment cxt L.start_block;
    let cxt = statement_list top cxt b in
    ipp_comment cxt L.end_block;
    cxt
  | Variable l ->
    variable_declaration top cxt l
              ...
              paren_group cxt 1 ( fun _ ->
                  let cxt, new_id =
                    ...
                    | Some ident_expression, _ ->
                      let cxt = pp_var_assign cxt id in
                      let cxt = expression ~level:1 cxt ident_expression in
                      space cxt;
                      comma cxt;
                    ...
                    | None , _ ->
                      let id = Ext_ident.create (Ident.name id ^ "_finish") in
                      let cxt = pp_var_assign cxt id in
                      expression ~level:15 cxt finish, Some id in
                  semi cxt;
                  space cxt;
                  let cxt = ident cxt id in
                  space cxt;
  ...
  | Continue s -> continue cxt s ; cxt
    (* newline cxt;  #2642 *)
  | Debugger ->  debugger_nl cxt; cxt
  | Break -> break_nl cxt; cxt

  | Return e ->
    begin match e.expression_desc with
      | Fun (is_method,  l, b, env) ->
        let cxt =
          pp_function ~is_method cxt ~fn_state:Is_return l b env in
        semi cxt; cxt
      | Undefined ->
        return_sp cxt;
        semi cxt;
  ...
  | Int_switch (e, cc, def) ->
    ...
    space cxt;
    brace_vgroup cxt 1 (fun _ ->
        let cxt = loop_case_clauses cxt (fun cxt i -> string cxt (string_of_int i) ) cc in
        match def with
        | None -> cxt
        | Some def ->
          group cxt 1  (fun _ ->
  ...
  | String_switch (e, cc, def) ->
    ...
    space cxt;
    brace_vgroup cxt 1 (fun _ ->
        let cxt = loop_case_clauses cxt (fun cxt s -> Js_dump_string.pp_string cxt.pp s) cc in
        match def with
        | None -> cxt
        | Some def ->
          group cxt 1 (fun _ ->
...
and function_body (cxt : cxt) (b : J.block) : unit =
  match b with
  | []     -> ()
  | [s]    ->
    ...
    | If (bool,
        ...
        ->
        ignore (statement false cxt {s with statement_desc = If(bool,then_, [])} : cxt)
    | Return {expression_desc = Undefined } -> ()
    | _ ->
...
and statement_list top cxt  b =
  iter_lst cxt b  (fun cxt s -> statement top cxt s )
    (if top then
      (fun cxt -> newline cxt; force_newline cxt )
      else newline
    )
...
and block cxt b =
  (* This one is for '{' *)
  brace_vgroup cxt 1 (fun _ -> statement_list false cxt b )
...
let string_of_block  (block : J.block) =
  ...
  let _ : cxt = statement_list true cxt block in
  flush cxt ();
...
let statement_list top scope pp b = (statement_list top { scope; pp } b).scope