...
let eval_rec_bindings_aux
  ...
  let rec bind_inits args acc =
    ...
    | (_id, None, _rhs) :: rem ->
      bind_inits rem acc
    | (Translmod.Ignore_loc _, _, _) :: rem ->
      bind_inits rem acc
  ...
  let rec patch_forwards args =
    ...
    | (_id, None, _rhs) :: rem ->
      patch_forwards rem
    | (Translmod.Ignore_loc _, _, _rhs) :: rem ->
      patch_forwards rem
    | (Id id, Some(_loc, shape), rhs) :: rem ->
      Lsequence(
        bs_update_mod [shape; Lvar id; rhs] Loc_unknown,
        patch_forwards rem)
  in
  bind_inits bindings
...
let eval_rec_bindings
  ...
  if  is_strict_or_all_functions bindings then
    Lambda.Lletrec
      (Ext_list.filter_map bindings (function (Id id,_,rhs) -> Some(id,rhs) | _ -> None),cont  )
  else
