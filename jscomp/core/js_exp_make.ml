...
let ocaml_fun
  ...
  make_expression ?loc ?comment (Fun (false, params,block, Js_fun_env.make ?immutable_mask len ))
...
let method_
    ...
    params block  : t =
  let len = List.length params in
  make_expression ?loc ?comment (Fun (true, params,block, Js_fun_env.make ?immutable_mask len ))
