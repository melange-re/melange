...
let subst (s : Lam.t Map_ident.t) lam =
  let rec subst_aux (x : Lam.t) : Lam.t =
    ...
    | Lapply { ap_func; ap_args; ap_info } ->
        Lam.apply (subst_aux ap_func) (List.map subst_aux ap_args) ap_info
    ...
    | Lletrec (decl, body) ->
        Lam.letrec (List.map subst_decl decl) (subst_aux body)
    | Lprim { primitive; args; loc } ->
        Lam.prim ~primitive ~args:(List.map subst_aux args) loc
    | Lglobal_module _ -> x
    | Lswitch (arg, sw) ->
    ...
    | Lstringswitch (arg, cases, default) ->
        Lam.stringswitch (subst_aux arg)
          (List.map subst_strcase cases)
          (subst_opt default)
    | Lstaticraise (i, args) -> Lam.staticraise i (List.map subst_aux args)
