...
let not_ loc x  : t =
  match x with
  | Lprim ({primitive = Pintcomp Cne } as prim)->
    ...
    | _ ->
      (match a with
       | Lprim {primitive = Pisout off; args = [Lconst(Const_int {i =  range}); Lvar xx] }
         ...
         | Lifthenelse(Lprim({primitive = Pintcomp Cne ; args = [Lvar j; Lconst _]} as b_pred)  , b_t, b_f)
           when Ident.same i j && eq_approx b_t c ->
           Lifthenelse(Lprim{b_pred with primitive = Pintcomp Ceq}, b_f, b_t)
         | Lprim({primitive = Pintcomp Cne ; args = [Lvar j; Lconst _] as args ; loc} )
         | Lprim(
