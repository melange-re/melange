...
let clear h =
  h.size <- 0;
  let len = Stdlib.Array.length h.data in
  for i = 0 to len - 1 do
    Stdlib.Array.unsafe_set h.data i Empty
  done

let reset h =
  h.size <- 0;
  h.data <- Stdlib.Array.make h.initial_size Empty
...
let resize indexfun h =
  ...
  if nsize < Sys.max_array_length then (
    ...
    let ndata_tail = Stdlib.Array.make nsize Empty in
    h.data <- ndata;
    ...
    let rec insert_bucket = function
      ...
      | Cons { key; next } as cell ->
          ...
          (match Stdlib.Array.unsafe_get ndata_tail nidx with
          | Empty -> Stdlib.Array.unsafe_set ndata nidx cell
          | Cons tail -> tail.next <- cell);
          Stdlib.Array.unsafe_set ndata_tail nidx cell;
          insert_bucket next
    in
    ...
    for i = 0 to nsize - 1 do
      match Stdlib.Array.unsafe_get ndata_tail i with
      | Empty -> ()
...
let iter h f =
  ...
  for i = 0 to Stdlib.Array.length d - 1 do
    do_bucket (Stdlib.Array.unsafe_get d i)
  done
...
let fold h init f =
  ...
  for i = 0 to Stdlib.Array.length d - 1 do
    accu := do_bucket (Stdlib.Array.unsafe_get d i) !accu
  done;
  !accu

let to_list set = fold set [] Stdlib.List.cons
...
let rec remove_bucket (h : _ t) (i : int) key ~(prec : _ bucket)
  ...
  | Cons { key = k; next } ->
      if eq_key k key then (
        h.size <- h.size - 1;
        match prec with
        | Empty -> Stdlib.Array.unsafe_set h.data i next
        | Cons c -> c.next <- next)
