    ...
    let rec insert_bucket = function
      ...
      | Cons { key; ord; data; next } ->
          ...
          Stdlib.Array.unsafe_set ndata nidx
            (Cons { key; ord; data; next = Stdlib.Array.unsafe_get ndata nidx });
          insert_bucket next
    in
...
let choose h =
  let rec aux arr offset len =
    if offset >= len then raise Not_found
      ...
      match Stdlib.Array.unsafe_get arr offset with
      | Empty -> aux arr (offset + 1) len
      | Cons { key = k; _ } -> k
  in
  aux h.data 0 (Stdlib.Array.length h.data)

let to_sorted_array h =
  if h.size = 0 then [||]
  else
    let v = choose h in
    let arr = Stdlib.Array.make h.size v in
    iter h (fun k _ i -> Stdlib.Array.unsafe_set arr i k);
    arr
