  ...
  let data, revData =
    ...
    let rec aux (row_fields : Parsetree.row_field list) =
      ...
      | tag :: rest ->
        ...
        | Rtag ({txt}, _, []) ->
          let name : string =
            match Ast_attributes.iter_process_bs_string_as tag.prf_attributes with
            | Some name ->
              has_bs_as := true;
              name
            | None -> txt in
...
let (-~) a b =
  app2 (Exp.ident {loc = noloc; txt = Ldot(Lident "Stdlib","-")})
    a b
let (+~) a b =
  app2 (Exp.ident {loc = noloc; txt = Ldot(Lident "Stdlib","+")})
    a b
                    ...
                    Typ.object_
                      (Ext_list.map label_declarations
                         (fun {pld_name ; pld_type } ->
                           Ast_compatible.object_field pld_name [] pld_type))
                      flag in
                  newTypeStr +?
                  [
