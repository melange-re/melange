...
module T = struct
  ...
  let row_field sub rf =
    ...
    | Rtag (l, b, tl) ->
        {
          rf with
          prf_desc = Rtag (map_loc sub l, b, List.map (sub.typ sub) tl);
          prf_attributes = sub.attributes sub rf.prf_attributes;
        }
    | Rinherit t ->
        {
          rf with
          prf_desc = Rinherit (sub.typ sub t);
          prf_attributes = sub.attributes sub rf.prf_attributes;
        }

  let object_field sub of_ =
    match of_.pof_desc with
    | Otag (l, t) ->
        {
          of_ with
          pof_desc = Otag (map_loc sub l, sub.typ sub t);
          pof_attributes = sub.attributes sub of_.pof_attributes;
        }
    | Oinherit t ->
        {
          of_ with
          pof_desc = Oinherit (sub.typ sub t);
          pof_attributes = sub.attributes sub of_.pof_attributes;
        }
...
module CT = struct
  ...
  let map sub { pcty_loc = loc; pcty_desc = desc; pcty_attributes = attrs } =
    ...
    | Pcty_open (({ popen_expr = lid } as ovf), ct) ->
        open_ ~loc ~attrs
          { ovf with popen_expr = map_loc sub lid }
  ...
  let map sub { pmty_desc = desc; pmty_loc = loc; pmty_attributes = attrs } =
    ...
    | Pmty_functor (mt1, mt2) ->
        functor_ ~loc ~attrs
          (match mt1 with
          | Unit -> Unit
          | Named (nm, mt1) -> Named (map_loc sub nm, sub.module_type sub mt1))
  ...
  let map_structure_item sub { pstr_loc = loc; pstr_desc = desc } =
    ...
    | Pstr_value (r, vbs) ->
        (* #if false then
           ...
           #else *)
        value ~loc r
          ((if r = Recursive then sub.value_bindings_rec
           else sub.value_bindings)
             sub vbs)
    (* #end *)
    ...
    | Pstr_exception ({ ptyexn_constructor } as ed) ->
        exception_ ~loc
          {
            ed with
            ptyexn_constructor =
...
module E = struct
  ...
  let map sub { pexp_loc = loc; pexp_desc = desc; pexp_attributes = attrs } =
    ...
    | Pexp_open (({ popen_expr = modexp } as ovf), e) ->
        open_ ~loc ~attrs
          { ovf with popen_expr = sub.module_expr sub modexp }
          (sub.expr sub e)
...
module CE = struct
  ...
  let map_field sub { pcf_desc = desc; pcf_loc = loc; pcf_attributes = attrs } =
    ...
    match desc with
    | Pcf_inherit (o, ce, s) ->
        inherit_ ~loc ~attrs o (sub.class_expr sub ce) (map_opt (map_loc sub) s)
...
let default_mapper =
  {
    ...
    open_description =
      (fun this { popen_expr; popen_override; popen_attributes; popen_loc } ->
        Opn.mk (map_loc this popen_expr) ~override:popen_override
          ~loc:(this.location this popen_loc)
          ~attrs:(this.attributes this popen_attributes));
    include_description =
