...
module Make (T : Reason_toolchain_conf.Toolchain) = struct
  ...
  let parse_implementation filename =
    let omp_ast = setup_lexbuf ~parser:T.implementation filename in
    let omp_ast =
      Reason_syntax_util.(
        apply_mapper_to_structure omp_ast
  ...
  let parse_interface filename =
    let omp_ast = setup_lexbuf ~parser:T.interface filename in
    Reason_toolchain.To_current.copy_signature omp_ast

  let parse_implementation_with_comments filename =
    let omp_ast, comments =
      setup_lexbuf ~parser:T.implementation_with_comments filename
    in
    let omp_ast =
      Reason_syntax_util.(
        apply_mapper_to_structure omp_ast
  ...
  let parse_interface_with_comments filename =
    let omp_ast, comments =
      setup_lexbuf ~parser:T.interface_with_comments filename
    in
  ...
  let format_implementation filename =
    format ~parser:T.implementation_with_comments
      ~printer:T.print_implementation_with_comments filename

  let format_interface filename =
    format ~parser:T.interface_with_comments
      ~printer:T.print_interface_with_comments filename
end
