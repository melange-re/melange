...
let has_bs_uncurry (attrs : Ast_attributes.t) =
  Ext_list.exists attrs (fun { attr_name = { txt; loc = _ }; _ } ->
          ...
          ->
            ...
            let new_arg_label, new_arg_types, output_tys =
              ...
              | Labelled name -> (
                  ...
                  | Fn_uncurry_arity _ ->
                      Location.raise_errorf ~loc
                        "The combination of %@obj, %@uncurry is not supported \
                         yet"
                  ...
                  | Unwrap ->
                      Location.raise_errorf ~loc
                        "%@obj label %s does not support %@unwrap arguments"
                        name)
              | Optional name -> (
    ...
    let arg_type_specs, new_arg_types_ty, arg_type_specs_length =
      let init : External_arg_spec.params * Ast_core_type.param_type list * int
        ...
        | Some obj -> (
            ...
            match arg_type with
            | Arg_cst _ ->
                Location.raise_errorf ~loc:obj.ptyp_loc
                  "%@as is not supported in %@send type "
            | _ ->
                (* more error checking *)
                ( [ { arg_label = Arg_empty; arg_type } ],
                  [
                    { label = Nolabel; ty = obj; attr = []; loc = obj.ptyp_loc };
                  ],
                  0 ))
        | None -> ([], [], 0)
      in
          ...
          let ( (arg_label : External_arg_spec.label_noname),
            ...
            | Optional s -> (
                ...
                | Poly_var _ ->
                    ...
                    Location.raise_errorf ~loc
                      "%@string does not work with optional when it has \
                       arities in label %s"
                      s
...
let handle_attributes_as_string (pval_loc : Location.t) (typ : Ast_core_type.t)
    (attrs : Ast_attributes.t) (pval_name : string) (prim_name : string) :
    response =
