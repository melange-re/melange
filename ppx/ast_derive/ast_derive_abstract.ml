...
let handleTdclsInStr ~light _rf tdcls =
  List.fold_right
    ~f:(fun tdcl sts ->
      let value_descriptions = handleTdcl ~light tdcl in
...
let handleTdclCstr (tdcl : Parsetree.type_declaration) :
    Parsetree.value_description list =
  let loc = tdcl.ptype_loc in
  let type_name = tdcl.ptype_name.txt in
  let core_type = U.core_type_of_type_declaration tdcl in
...
let handleCstrTdclsInStr _rf tdcls =
  List.fold_right
    ~f:(fun tdcl sts ->
      let value_descriptions = handleTdclCstr tdcl in
      List.map ~f:Str.primitive value_descriptions @ sts)
    tdcls ~init:[]

let handleCstrTdclsInSig _rf tdcls =
  List.fold_right
    ~f:(fun tdcl sts ->
      let value_descriptions = handleTdclCstr tdcl in
      List.map ~f:Sig.value value_descriptions @ sts)
...
let handleGettersSettersTdcls ~light (tdcl : Parsetree.type_declaration) :
    Parsetree.value_description list =
  let loc = tdcl.ptype_loc in
  let core_type = U.core_type_of_type_declaration tdcl in
  match tdcl.ptype_kind with
...
let handleGettersSettersTdclsInStr ~light _rf tdcls =
  List.fold_right
    ~f:(fun tdcl sts ->
      let value_descriptions = handleGettersSettersTdcls tdcl ~light in
      List.map ~f:Str.primitive value_descriptions @ sts)
    tdcls ~init:[]

let handleGettersSettersTdclsInSig ~light _rf tdcls =
  List.fold_right
    ~f:(fun tdcl sts ->
      let value_descriptions = handleGettersSettersTdcls ~light tdcl in
      List.map ~f:Sig.value value_descriptions @ sts)
    tdcls ~init:[]
