...
let rec sortedLengthAuxMore xs prec acc len lt =
  if acc >= len then acc
    ...
    if lt v prec [@u] then sortedLengthAuxMore xs v (acc + 1) len lt else acc

let rec sortedLengthAuxLess xs prec acc len lt =
  if acc >= len then acc
  else
    let v = A.getUnsafe xs acc in
    if lt prec v [@u] then sortedLengthAuxLess xs v (acc + 1) len lt else acc
...
let strictlySortedLengthU xs lt =
  ...
  | _ ->
      ...
      if lt x0 x1 [@u] then sortedLengthAuxLess xs x1 2 len lt
      else if lt x1 x0 [@u] then -sortedLengthAuxMore xs x1 2 len lt
      else 1
...
let rec isSortedAux a i cmp last_bound =
  ...
  else if (cmp (A.getUnsafe a i) (A.getUnsafe a (i + 1)) [@u]) <= 0 then
...
let insertionSort src srcofs dst dstofs len cmp =
  for i = 0 to len - 1 do
    ...
    while
      j.contents >= dstofs && (cmp (A.getUnsafe dst j.contents) e [@u]) > 0
    do
