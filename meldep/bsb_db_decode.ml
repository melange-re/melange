...
let read_build_cache ~dir : t =
  let all_content =
    Ext_io.load_file (Filename.concat dir Literals.bsbuild_cache)
  in
  decode all_content

type module_info = { case : bool; (* which is Bsb_db.case*) dir_name : string }

let find_opt ({ content = whole; _ } as db : t) ~(kind : [ `impl | `intf ]) lib
    (key : string) : module_info option =
...
let find db ~(kind : [ `impl | `intf ]) dependent_module is_not_lib_dir =
  ...
  | Some _ -> opt
  | None ->
      if is_not_lib_dir then find_opt ~kind db false dependent_module else None
