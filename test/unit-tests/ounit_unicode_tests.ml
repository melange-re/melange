...
let empty_segment { String_interp.Private.content; _ } =
  ...
  OUnit.assert_equal
    (List.map
       (fun ({
              ...
              String_interp.Private.segment) -> (a, b, kind, content))
  ...
  let segments =
    List.map
      (fun ({
             ...
             String_interp.Private.segment) -> (la, a, lb, b, kind, content))
      (String_interp.Private.transform_test a
      |> List.filter (fun x -> not @@ empty_segment x))
  in
  OUnit.assert_equal segments b

let varParen : String_interp.kind = Var (2, -1)
let var : String_interp.kind = Var (1, 0)
